-- Adds client portal confirmation deadlines and updates availability semantics.
-- Contains no exported rows or production data.
-- Does not delete appointments, deposits, payments or client records.
-- Keeps the existing create_appointment_transaction signature.
...
create index if not exists appointments_portal_deadline_idx
  on public.appointments (
    booking_source,
    confirmation_status,
    confirmation_deadline_at
  )
  where confirmation_deadline_at is not null;

create or replace function public.client_portal_confirmation_deadline(
...
as $$
  select
    ...
    case
      when extract(
        hour from timezone('America/Merida', coalesce(p_created_at, now()))
      ) >= 5
      and extract(
        hour from timezone('America/Merida', coalesce(p_created_at, now()))
      ) < 22
...
create or replace function public.appointment_blocks_availability(
  p_status text,
...
as $$
  select
    lower(coalesce(p_status, '')) not in (
      'cancelada',
      'cancelado',
      'cancelled',
      'canceled',
    ...
    and lower(coalesce(p_confirmation_status, '')) not in (
      'cancelada',
      'cancelado',
      'cancelled',
      'canceled',
    ...
    and not (
      lower(coalesce(p_booking_source, '')) in (
        'cliente_portal',
        'client_portal'
      )
      and lower(coalesce(p_confirmation_status, 'pendiente')) = 'pendiente'
      and p_confirmation_deadline_at is not null
      and coalesce(p_now, now()) >= p_confirmation_deadline_at
    );
$$;

revoke all on function public.client_portal_confirmation_deadline(
  timestamptz
) from public, anon, authenticated;

grant execute on function public.client_portal_confirmation_deadline(
  timestamptz
) to service_role;

revoke all on function public.appointment_blocks_availability(
  text,
  text,
  text,
  timestamptz,
...
grant execute on function public.appointment_blocks_availability(
  text,
  text,
  text,
  timestamptz,
...
declare
  ...
  v_confirmation_deadline_at timestamptz := null;
...
begin
  ...
  if v_source = 'client_portal' then
    v_confirmation_deadline_at :=
      public.client_portal_confirmation_deadline(now());
  end if;
  ...
  begin
    ...
    if
      ...
      where existing_service.staff_id = p_staff_id
        ...
        and lower(coalesce(existing_service.status, 'agendado')) not in (
          'cancelada',
          'cancelado',
          'cancelled',
          'canceled',
        ...
        and public.appointment_blocks_availability(
          existing_appointment.status,
          existing_appointment.confirmation_status,
          existing_appointment.booking_source,
          existing_appointment.confirmation_deadline_at,
          now()
        )
        and p_appointment_date + p_start_time <
    ...
    v_result := jsonb_build_object(
      ...
      'staffId', p_staff_id,
      'confirmationDeadlineAt', v_confirmation_deadline_at,
      'errorCode', null,
      'errorMessage', null
    );
