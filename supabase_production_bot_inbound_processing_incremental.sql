...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bot_conversations_provider_check'
      AND conrelid = 'public.bot_conversations'::regclass
  ) THEN
    ALTER TABLE public.bot_conversations
      ADD CONSTRAINT bot_conversations_provider_check
      CHECK (provider IS NULL OR provider IN ('meta_whatsapp'));
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bot_messages_provider_check'
      AND conrelid = 'public.bot_messages'::regclass
  ) THEN
    ALTER TABLE public.bot_messages
      ADD CONSTRAINT bot_messages_provider_check
      CHECK (provider IS NULL OR provider IN ('meta_whatsapp'));
  END IF;
END $$;
